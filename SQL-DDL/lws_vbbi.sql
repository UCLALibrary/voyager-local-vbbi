...
--  , card_number char(2) := '01'
  , action_code char(1) := 'A'
  , vendor_number char(10)
  , invoice_number char(23)
  , invoice_date char(6)
  , batch_sequence char(2) := '99'
  , batch_number char(6) := '999910'
  , po_reference char(10)
  , encumbrance_release char(1) := ' ' -- z20 default is blank, set at z21 level
  , invoice_total_amount number -- total to *vendor*, not necessarily total paid
  , invoice_type char(1)
  , sales_tax_amount number := 0 -- amount of tax paid to the vendor, not the state
--  , voy_vendor_tax_amount number := 0 -- tax paid to vendor per voyager, for comparison
  , terms_code char(2) := '99'
  , discount_amount number := 0 -- used only for term discounts, which we will rarely (never?) use
  , pac_invoice_total number := 0 -- real total paid, vendor and state
  , voy_invoice_total number := 0 -- total paid per voyager, for comparison
  , invoice_due_date char(6)
  , retention_amount char(15) := lws_utility.get_blanks(15) -- always blank
  , adjustment_code char(1) := ' '
  , income_reporting char(1) := ' '
  , income_reporting_code char(1) := ' '
  , hold_cycle char(2) := '00' -- changed from '0 ' per Prasad Naik 20090508
  , reason char(2) := '  '
  , release_code char(1) := ' '
  , check_dist_code char(1) := ' '
  , pck_segment char(4) := 'UCLA'
  , po_control_key char(12) := 'NONE' || lws_utility.get_blanks(8)
  , tax_state_code char(2) := 'CA'
  , line_1 char(80)
  , line_2 char(80)
  , line_3 char(80)
  );
  ...
  , vendor_number char(10)
  , invoice_number char(23)
  , transaction_date char(6) := lws_utility.get_blanks(6)
...
--  , card_number char(2) := '01'
  ...
  , invoice_number char(23)
  , line_number char(4)
  , invoiced_percent char(8) := lws_utility.get_blanks(8)
  , invoiced_quantity char(15) := lws_utility.get_blanks(15)
--  , invoiced_amount char(15)
  , invoiced_amount number
  , fau_location char(1)
  , fau_account char(6)
  , fau_cost_center char(2)
  , fau_fund char(5)
  , fau_project char(6)
  , fau_sub char(2)
  , fau_object char(4)
  , fau_source char(6)
  , po_reference char(10) := lws_utility.get_blanks(10)
  , acct_eff_date char(6) := lws_utility.get_blanks(6)
  , line_1 char(80)
  , line_2 char(80)
  , line_3 char(80)
  );
  type z41_collection is table of z41_record index by binary_integer;

  type z21_record is record (
    transaction_number char(3) := 'Z21'
  , screen_number char(2) := '01'
--  , card_number char(2) := '01'
  , action_code char(1) := 'A'
  , vendor_number char(10)
  ...
  , line_number char(4)
  , line_code char(3)
  , quantity char(15) := lws_utility.get_blanks(15) -- use line_amount instead
  , unit_price char(15) := lws_utility.get_blanks(15) -- use line_amount instead
  , line_amount number
  , description char(55)
  , sales_tax_code char(1)
  , tax_rate_group_code char(1)
  , fau_location char(1)
  , fau_account char(6)
  , fau_cost_center char(2)
  , fau_fund char(5)
  ...
  , fau_source char(6)
  , po_reference char(10)
  , uc_property_number char(16) := lws_utility.get_blanks(16)
  , fst_status_code char(1) := 'E'
  , po_line_number char(4) := lws_utility.get_blanks(4)
  , encumbrance_release char(1)
  , line_1 char(80)
  , line_2 char(80)
  , line_3 char(80)
  , z41 z41_collection
  );
  type z21_collection is table of z21_record index by binary_integer;

  type pac_record is record (
    z20 z20_record
  , z21 z21_collection
  , z25 z25_record
  );
  ...
  function format_z20_line_3(p_z20 z20_record) return varchar2;
  function format_z21_line_1(p_z21 z21_record) return varchar2;
  function format_z21_line_2(p_z21 z21_record) return varchar2;
  function format_z21_line_3(p_z21 z21_record) return varchar2;
  function format_z25 (p_z25 z25_record) return varchar2;
  function format_z41_line_1 (p_z41 z41_record) return varchar2;
  function format_z41_line_2 (p_z41 z41_record) return varchar2;
  function format_z41_line_3 (p_z41 z41_record) return varchar2;
  function build_z20 (p_voy_record voyager_record) return z20_record;
  function build_z20_delete_record (p_invoice_id integer) return z20_record;
  function build_z21 (p_voy_record voyager_record, p_line_number integer, p_line_code varchar2 := NULL) return z21_record;
  function build_z25 (p_voy_record voyager_record) return z25_record;
  function build_z41 (p_voy_record voyager_record, p_line_number integer) return z41_record;
  function get_special_tax_fund(p_description varchar2) return char;
  function get_tax_rate(p_invoice_date varchar2) return number;
  function is_taxable(p_z21 z21_record) return boolean;
  function needs_line_item(p_voy_record voyager_record) return boolean;
  procedure build_delete_invoices;
  procedure build_invoice_data(p_invoice_id integer, p_prod_mode boolean := TRUE);
  procedure build_vbbi_file;
  procedure reset_invoice(p_invoice_id integer);
  procedure store_log_record(p_invoice_id integer, p_message varchar2, p_amount number);
  procedure store_pac_record(p_invoice_id integer, p_pac_invoice pac_record);
  procedure store_z20_delete_record(p_invoice_id integer, p_z20_record z20_record);
  procedure update_pac_record_totals(p_invoice_id integer, p_voy_vendor_tax_amount number, p_pac_invoice in out pac_record, p_prod_mode boolean := TRUE);
  procedure update_z21(p_z21 in out z21_record, p_line_amount number);
end lws_vbbi;
/
...
create or replace package body vger_support.lws_vbbi as
  ...
  begin
    ...
    case
      when p_invoice_date >= '130101' then rate := .0900;
...
select * from user_errors where name = 'LWS_VBBI';
